...
export class SchemaIngestionService {
	...
	/**
	 * Ingest schema as JSON-structured descriptions with semantic chunking
	 * Creates separate chunks for: table overview, column details, and relationships
	 * This improves RAG precision by avoiding noise from large monolithic chunks
	 */
	async ingestSchemaJsonDescriptions(
		...
		this.logger.log(
			`Starting JSON schema ingestion with semantic chunking for tenant: ${tenantId}, db: ${dbKey}`,
		);
		...
		for (const table of tables) {
			...
			aiChunks.push({
				...
				dbKey,
				content: JSON.stringify(tableOverview, null, 2),
			});

			// Chunk 2-N: Column Details (one chunk per column for precise retrieval)
			for (const column of columns) {
				...
				const columnChunk = {
					...
					sample_values: (samples[column.column_name] || []).slice(0, 3),
				};
				aiChunks.push({
					tenantId,
					collection: 'schema' as AiChunkCollection,
					dbKey,
					content: JSON.stringify(columnChunk, null, 2),
				});
			}

			// Chunk N+1: Relationships (one chunk per relationship)
			for (const fk of foreignKeys) {
				...
				aiChunks.push({
					...
					collection: 'schema' as AiChunkCollection,
					dbKey,
					content: JSON.stringify(relationshipChunk, null, 2),
				});
			}
		}

		this.logger.log(
			`Generated ${aiChunks.length} semantic schema chunks (table overviews, column details, relationships)`,
		);
