/**
 * Prompt templates for ConversationalAgent
 */

export interface ConversationalPromptParams {
	recentMessages?: string;
	query: string;
	isFirstMessage: boolean;
}

export function buildConversationalPrompt(params: ConversationalPromptParams): string {
	const { recentMessages, query, isFirstMessage } = params;
	return `
...
export const DEFAULT_GREETING_MESSAGE = `Xin chào! 👋 Tôi là AI Assistant của Trustay, rất vui được trò chuyện với bạn!

Tôi có thể giúp bạn tìm hiểu về dữ liệu phòng trọ, thống kê doanh thu, thông tin người dùng và nhiều thứ khác.

Bạn muốn tìm hiểu điều gì? 😊`;

export const DEFAULT_SEARCH_MESSAGE = `Tôi sẽ tìm kiếm thông tin cho bạn ngay! 🔍`;
