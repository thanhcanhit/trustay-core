...
describe('ListingService', () => {
	...
	describe('findCombinedListings', () => {
		it('should return combined room listings and roommate seeking posts', async () => {
			// Arrange
			const query: ListingQueryDto = {
				page: 1,
				limit: 10,
			...
			(mockPrismaService.room.findMany as jest.Mock).mockResolvedValue(mockRooms as any);
			(mockPrismaService.roommateSeekingPost.findMany as jest.Mock).mockResolvedValue(
				mockRoommatePosts as any,
			);
			(mockPrismaService.room.count as jest.Mock).mockResolvedValue(1);
			(mockPrismaService.roommateSeekingPost.count as jest.Mock).mockResolvedValue(1);
			(mockPrismaService.district.findUnique as jest.Mock).mockResolvedValue({
				name: 'District 1',
			});
			(mockPrismaService.province.findUnique as jest.Mock).mockResolvedValue({
			...
			// Act
			const result = await service.findCombinedListings(query, { isAuthenticated: false });
			...
			expect(result.stats.totalRooms).toBe(1);
			expect(result.stats.totalRoommateSeekingPosts).toBe(1);

			// Check that we have both types
			const roomItems = result.data.filter((item) => item.type === 'room');
			const roommateItems = result.data.filter((item) => item.type === 'roommate_seeking');
			expect(roomItems).toHaveLength(1);
			expect(roommateItems).toHaveLength(1);
		});

		it('should apply user preferences for combined search', async () => {
			// Arrange
			const tenant = await authUtils.createDefaultTenant();
			const query: ListingQueryDto = {
				page: 1,
			...
			(mockPrismaService.roommateSeekingPost.findMany as jest.Mock).mockResolvedValue([]);
			(mockPrismaService.room.count as jest.Mock).mockResolvedValue(0);
			(mockPrismaService.roommateSeekingPost.count as jest.Mock).mockResolvedValue(0);

			// Act
			await service.findCombinedListings(query, { isAuthenticated: true, userId: tenant.id });
				...
				expect.objectContaining({
					where: expect.objectContaining({
						...
						pricing: expect.objectContaining({
							basePriceMonthly: expect.objectContaining({
								gte: 800000,
								lte: 6000000,
							}),
						}),
			...
			expect(mockPrismaService.roommateSeekingPost.findMany).toHaveBeenCalledWith(
				expect.objectContaining({
					where: expect.objectContaining({
						externalProvinceId: 1,
						externalDistrictId: 1,
						monthlyRent: {
							gte: 800000, // 0.8 * 1000000
							lte: 6000000, // 1.2 * 5000000
						},
					}),
				}),
			);
