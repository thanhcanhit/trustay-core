...
async function importVietnameseAdministrativeData() {
  ...
  try {
    const workbook = XLSX.readFile(join(process.cwd(), "data", "data.xlsx"));
    const sheetName = workbook.SheetNames[0];
    const sheet = workbook.Sheets[sheetName];
    const data: VNAdminData[] = XLSX.utils.sheet_to_json(sheet);
    ...
    const districts = new Map<string, { code: string; name: string; provinceCode: string }>();
    ...
    for (const [code, district] of districts) {
      try {
        ...
        if (province) {
          ...
          console.warn(`⚠️ Không tìm thấy province cho district: ${district.name} (${district.provinceCode})`);
        }
      } catch (error) {
        ...
        if (row["Mã PX"] && row["Phường Xã"]) {
          try {
            ...
            if (district) {
              ...
              console.warn(`⚠️ Không tìm thấy district cho ward: ${row["Phường Xã"]} (${row["Mã QH"]})`);
            }
          } catch (error) {
...
async function validateDataBeforeImport() {
  ...
  try {
    ...
    const requiredColumns = ["Tỉnh Thành Phố", "Mã TP", "Quận Huyện", "Mã QH", "Phường Xã", "Mã PX", "Cấp"];
...
export { importVietnameseAdministrativeData, validateDataBeforeImport };